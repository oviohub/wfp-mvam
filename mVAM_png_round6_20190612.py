...
DATA_DIR = '/Users/shaquillewyanque/Documents/WFP - mVAM'
ADMIN_AREA_FILE = '/Users/shaquillewyanque/Documents/WFP - mVAM/Sampling frame_UNWFP May 2019_GeoCode.xlsx'
...
def download_data(survey_name, save=True, verbose=False):
	...
	token = ''
	response = requests.get('https://kc.humanitarianresponse.info/api/v1/forms',
							headers={'Authorization': token})
#    print response.content
	formid = None
	for item in response.json():
		# print(item['title'], item['formid'])
		if item['title'] == survey_name:
...
def clean_data(raw_df, save=True):
	raw_df[["RESPConsent", "Complete"]] = raw_df[["RESPConsent", "Complete"]].apply(pd.to_numeric)
	...
	if save:
		duplicates.to_csv(os.path.join(DATA_DIR, DUPLICATES_FILE), header=['Count'])
	...
	enumerator_crosstab.loc['Total'] = pd.Series(enumerator_crosstab['Completed'].sum(), index = ['Completed'])
	if save:
...
def count_target_by_llg(df, save=True):
	...
	llg_count = pd.DataFrame(df.pivot_table(index='ADMIN3Code', aggfunc='size'))
	llg_count.columns = ['Completed']
	
	# merge with llg data
	admin_areas = pd.read_excel(ADMIN_AREA_FILE, sheet_name='Master Sheet', dtype=str)
	df = pd.merge(admin_areas, llg_count, how='left', left_on=['GEOCODE'], right_on=['ADMIN3Code'])
